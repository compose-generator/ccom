...
Lexer::Lexer(bool isSingleStatement, const std::string &fileInput, const std::string &inputCommentLineIdentifiers,
  ...
  unsigned int maxLookahead = std::max({commentLineIdentifier.length(), commentBlockOpenIdentifier.length(),
                                        commentBlockCloseIdentifier.length(), commentPayloadIdentifier.length()}) +
                              1;
  ...
  currentContext =
      isSingleStatement || isLookAheadCommentLineIdentifier() || isLookAheadCommentBlockOpenIdentifier() ? SECTION : ARBITRARY;
...
Token Lexer::getLookahead() { return curTok; }
...
Token Lexer::consumeSection() {
  ...
  if (isLookAheadCommentLineIdentifier())
    return consumeCommentLineIdentifier();
  if (isLookAheadCommentBlockOpenIdentifier())
    return consumeCommentBlockOpenIdentifier();
  if (isLookAheadCommentBlockCloseIdentifier())
    return consumeCommentBlockCloseIdentifier();
  if (isLookAheadCommentPayloadIdentifier())
    return consumeCommentPayloadIdentifier();
  ...
  case '>':
    return consumeGreaterOrGreaterEqual();
  case '<':
    return consumeLessOrLessEqual();
  case '.':
    return consumeDot();
  case '{':
...
Token Lexer::consumeEOF() { return constructToken(TOK_EOF); }
...
Token Lexer::consumeIdentifierOrKeyword() {
  ...
  if (identifierName == "if")
    return constructToken(TOK_IF);
  if (identifierName == "has")
    return constructToken(TOK_HAS);
  if (identifierName == "not")
    return constructToken(TOK_NOT);
  if (identifierName == "true")
    return constructToken(TOK_TRUE);
  if (identifierName == "false")
    return constructToken(TOK_FALSE);
  if (identifierName == "contains")
    return constructToken(TOK_CONTAINS);
...
bool Lexer::isEOF() { return reader.getLookahead() == EOF; }
...
Token Lexer::constructToken(TokenType type) { return Token(type, tokenStartLineNum, tokenStartColNum); }

Token Lexer::constructToken(TokenType type, std::string value) {
  return {type, std::move(value), tokenStartLineNum, tokenStartColNum};
}
