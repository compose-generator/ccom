...
Reader::Reader(std::string fileInput, unsigned int maxLookahead) : fileInput(std::move(fileInput)), maxLookahead(maxLookahead) {
  // Check maxLookahead validity
  if (maxLookahead < 1)
    throw MaxLookaheadException(maxLookahead);

  // Load first char into buffer
  advance();

  // Set colNum to 1, if the advance was not successful due to an empty
  // fileInput
  if (colNum == 0)
    colNum = 1;
}
...
int Reader::getLookahead() const { return curChar; }

std::string Reader::getLookaheadMultiple() {
  // inputStringPos represents the position to which we have read to. e.g.:
  // "Test", inputStringPos: 2, we have read "Te"
  int length = std::min((int)fileInput.length() - inputStringPos + 1, (int)maxLookahead);
  return fileInput.substr(inputStringPos - 1, length);
}

unsigned int Reader::getLineNum() const { return lineNum; }

unsigned int Reader::getColNum() const { return colNum; }